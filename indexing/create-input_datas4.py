...
def write_output(f, row, index):
    ...
    outdata = "以下は物件IDが" +str(id)+"の詳細情報である。\n"
    #outdata += "・物件ID: " + str(id) + '\n'
    outdata += "・" + names[0] + ': ' + row[0] + '\n'
    #print('index='+str(index)+' '+row[5])
    ...
    outdata += "・" + names[416] + ': ' + row[416] + '\n'
    outdata += "・" + "物件URL" + ': ' + 'https://www.cjs.ne.jp/chintai/detail/' + str(row[416]) + ".html\n"
...
with open('tsuruhashi2.csv','r', encoding='utf_8_sig') as f:
    reader = csv.reader(f)
